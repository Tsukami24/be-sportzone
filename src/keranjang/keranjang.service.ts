...
export class KeranjangService {
  constructor(
    @InjectRepository(Keranjang)
    private readonly cartRepo: Repository<Keranjang>,
    @InjectRepository(KeranjangItem)
    private readonly itemRepo: Repository<KeranjangItem>,
  ...
  async addItem(userId: string, dto: AddItemDto): Promise<Keranjang> {
    ...
    const existing = (cart.items || []).find(
      (i) =>
        i.produk_id === dto.produk_id &&
        (i.produk_varian_id || null) === (dto.produk_varian_id || null),
    );
    if (existing) {
  ...
  async updateItem(
    ...
    const item = await this.itemRepo.findOne({
      where: { id: itemId, keranjang_id: cart.id },
    });
